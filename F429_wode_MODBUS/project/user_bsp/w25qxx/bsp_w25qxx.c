...
void W25QXX_WriteEnable(W25QXX_HandleTypedef* spiFlash_x) {
    W25QXX_CS_ON(spiFlash_x); //使能器件
    W25QXX_ReadWriteOneByte(spiFlash_x, W25X_WriteEnable);     //发送写使能指令
    W25QXX_CS_OFF(spiFlash_x); //取消片选
}
...
void W25QXX_WriteDisable(W25QXX_HandleTypedef* spiFlash_x) {
    W25QXX_CS_ON(spiFlash_x); //使能器件
    W25QXX_ReadWriteOneByte(spiFlash_x, W25X_WriteDisable);     //发送写禁止指令
    W25QXX_CS_OFF(spiFlash_x); //取消片选
}
...
u8 W25QXX_Read_SR(W25QXX_HandleTypedef* spiFlash_x, u8 regno) {
    ...
    W25QXX_CS_ON(spiFlash_x); //使能器件
    W25QXX_ReadWriteOneByte(spiFlash_x, command);       //发送读取状态寄存器命令
    byte = W25QXX_ReadWriteOneByte(spiFlash_x, 0Xff); //读取一个字节
    W25QXX_CS_OFF(spiFlash_x); //取消片选
    return byte;
}
...
void W25QXX_Write_SR(W25QXX_HandleTypedef* spiFlash_x, u8 regno, u8 sr) {
    ...
    W25QXX_CS_ON(spiFlash_x);   //使能器件
    W25QXX_ReadWriteOneByte(spiFlash_x, command);         //发送写取状态寄存器命令
    W25QXX_ReadWriteOneByte(spiFlash_x, sr);                      //写入一个字节
    W25QXX_CS_OFF(spiFlash_x);  //取消片选
}
...
void W25QXX_WaitBusy(W25QXX_HandleTypedef* spiFlash_x) {
    while((W25QXX_Read_SR(spiFlash_x, 1) & 0x01) == 0x01); // 等待BUSY位清空
}
...
void W25QXX_ChipReset(W25QXX_HandleTypedef* spiFlash_x) {
    if(spiFlash_x->ready == 0) {
        ...
        W25QXX_CS_ON(spiFlash_x);   //使能器件
        W25QXX_ReadWriteOneByte(spiFlash_x, W25X_ResetEnable);       //发送重置命令
        W25QXX_CS_OFF(spiFlash_x);  //取消片选
        W25QXX_WaitBusy(spiFlash_x);   				   //等待芯片重置结束
        W25QXX_CS_ON(spiFlash_x);   //使能器件
        W25QXX_ReadWriteOneByte(spiFlash_x, W25X_ResetMemory);       //发送重置命令
        W25QXX_CS_OFF(spiFlash_x);  //取消片选
        W25QXX_WaitBusy(spiFlash_x);   				   //等待芯片重置结束
    }
}
...
void W25QXX_PowerDown(W25QXX_HandleTypedef* spiFlash_x) {
    W25QXX_CS_ON(spiFlash_x);   //使能器件
    W25QXX_ReadWriteOneByte(spiFlash_x, W25X_PowerDown);    //发送掉电命令
    W25QXX_CS_OFF(spiFlash_x);  //取消片选
    HAL_Delay(3);                            //等待TPD
}
...
void W25QXX_WAKEUP(W25QXX_HandleTypedef* spiFlash_x) {
    W25QXX_CS_ON(spiFlash_x);   //使能器件
    W25QXX_ReadWriteOneByte(spiFlash_x, W25X_ReleasePowerDown); //  发送唤醒指令
    W25QXX_CS_OFF(spiFlash_x);  //取消片选
    HAL_Delay(3);                                //等待TRES1
}
...
void W25QXX_ReadID(W25QXX_HandleTypedef* spiFlash_x) {
    u16 Temp = 0;
    W25QXX_CS_ON(spiFlash_x);   //使能器件
    W25QXX_ReadWriteOneByte(spiFlash_x, 0x90); //发送读取ID命令
...
void W25QXX_Init(W25QXX_HandleTypedef* spiFlash_x) {
    ...
    if(spiFlash_x->ready != 0) {
        ...
        W25QXX_CS_OFF(spiFlash_x);  //取消片选
        W25QXX_ReadID(spiFlash_x);	      //读取FLASH ID.
        if(spiFlash_x->W25Qxx_Type == W25Q256) {            //SPI FLASH为W25Q256
            temp = W25QXX_Read_SR(spiFlash_x, 3);           //读取状态寄存器3，判断地址模式
            if((temp & 0X01) == 0) {		    //如果不是4字节地址模式,则进入4字节地址模式
                W25QXX_CS_ON(spiFlash_x);   //使能器件
                W25QXX_ReadWriteOneByte(spiFlash_x, W25X_Enable4ByteAddr); //发送进入4字节地址模式指令
                W25QXX_CS_OFF(spiFlash_x);  //取消片选
            }
        }
    }
...
void W25QXX_Read(W25QXX_HandleTypedef* spiFlash_x, u8* pBuffer, u32 ReadAddr, u16 NumByteToRead) {
    ...
    if(spiFlash_x->ready == 0) {
        ...
        W25QXX_CS_ON(spiFlash_x);   //使能器件
        W25QXX_ReadWriteOneByte(spiFlash_x, W25X_ReadData);     //发送读取命令
        if(spiFlash_x->W25Qxx_Type == W25Q256) {            //如果是W25Q256的话地址为4字节的，要发送最高8位
            W25QXX_ReadWriteOneByte(spiFlash_x, (u8)((ReadAddr) >> 24));
        }
        W25QXX_ReadWriteOneByte(spiFlash_x, (u8)((ReadAddr) >> 16)); //发送24bit地址
        W25QXX_ReadWriteOneByte(spiFlash_x, (u8)((ReadAddr) >> 8));
        W25QXX_ReadWriteOneByte(spiFlash_x, (u8)ReadAddr);
        for(i = 0; i < NumByteToRead; i++) {
            pBuffer[i] = W25QXX_ReadWriteOneByte(spiFlash_x, 0XFF); //循环读数
        }
        W25QXX_CS_OFF(spiFlash_x);  //取消片选
    }
}
...
void W25QXX_Write_Page(W25QXX_HandleTypedef* spiFlash_x, u8* pBuffer, u32 WriteAddr, u16 NumByteToWrite) {
    ...
    if(spiFlash_x->ready == 0) {
        ...
        W25QXX_WriteEnable(spiFlash_x);                  //SET WEL
        W25QXX_CS_ON(spiFlash_x);   //使能器件
        W25QXX_ReadWriteOneByte(spiFlash_x, W25X_PageProgram);  //发送写页命令
        if(spiFlash_x->W25Qxx_Type == W25Q256) {            //如果是W25Q256的话地址为4字节的，要发送最高8位
            W25QXX_ReadWriteOneByte(spiFlash_x, (u8)((WriteAddr) >> 24));
        }
        W25QXX_ReadWriteOneByte(spiFlash_x, (u8)((WriteAddr) >> 16)); //发送24bit地址
        W25QXX_ReadWriteOneByte(spiFlash_x, (u8)((WriteAddr) >> 8));
        W25QXX_ReadWriteOneByte(spiFlash_x, (u8)WriteAddr);
        for(i = 0; i < NumByteToWrite; i++) {
            W25QXX_ReadWriteOneByte(spiFlash_x, pBuffer[i]);  //循环写数
        }
        W25QXX_CS_OFF(spiFlash_x);  //取消片选
        W25QXX_WaitBusy(spiFlash_x);					   //等待写入结束
    }
}
...
void W25QXX_Write(W25QXX_HandleTypedef* spiFlash_x, u8* pBuffer, u32 WriteAddr, u16 NumByteToWrite) {
    ...
    while(1) {
        W25QXX_Read(spiFlash_x, W25Q256_BUF, secpos * 4096, 4096); //读出整个扇区的内容
        ...
        if(i < secremain) { //需要擦除
            W25QXX_SectorErase(spiFlash_x, secpos); //擦除这个扇区
            for(i = 0; i < secremain; i++) { //复制
                W25Q256_BUF[i + secoff] = pBuffer[i];
            }
            W25QXX_Write_NoCheck(spiFlash_x, W25Q256_BUF, secpos * 4096, 4096); //写入整个扇区

        } else {
            W25QXX_Write_NoCheck(spiFlash_x, pBuffer, WriteAddr, secremain); //写已经擦除了的,直接写入扇区剩余区间.
        }
        if(NumByteToWrite == secremain) {
            break;//写入结束了
        } else { //写入未结束
...
void W25QXX_ChipErase(W25QXX_HandleTypedef* spiFlash_x) {
    ...
    W25QXX_WaitBusy(spiFlash_x);
    W25QXX_CS_ON(spiFlash_x);   //使能器件
    W25QXX_ReadWriteOneByte(spiFlash_x, W25X_ChipErase);       //发送片擦除命令
    W25QXX_CS_OFF(spiFlash_x);  //取消片选
    W25QXX_WaitBusy(spiFlash_x);   				   //等待芯片擦除结束
}
...
void W25QXX_SectorErase(W25QXX_HandleTypedef* spiFlash_x, u32 Dst_Addr) {
    ...
    if(spiFlash_x->ready == 0) {
        ...
        W25QXX_WaitBusy(spiFlash_x);
        W25QXX_CS_ON(spiFlash_x);   //使能器件
        W25QXX_ReadWriteOneByte(spiFlash_x, W25X_SectorErase);  //发送扇区擦除指令
        if(spiFlash_x->W25Qxx_Type == W25Q256) {            //如果是W25Q256的话地址为4字节的，要发送最高8位
            W25QXX_ReadWriteOneByte(spiFlash_x, (u8)((Dst_Addr) >> 24));
        }
        W25QXX_ReadWriteOneByte(spiFlash_x, (u8)((Dst_Addr) >> 16)); //发送24bit地址
        W25QXX_ReadWriteOneByte(spiFlash_x, (u8)((Dst_Addr) >> 8));
        W25QXX_ReadWriteOneByte(spiFlash_x, (u8)Dst_Addr);
        W25QXX_CS_OFF(spiFlash_x);  //取消片选
        W25QXX_WaitBusy(spiFlash_x);   				    //等待擦除完成
    }
}
